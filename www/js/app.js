...
lab2.controller('player1', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state ,$stateParams, navigation) {
  /* TO DO: Create processMove factory instance and place on local scope */
  $scope.processMove = processMove();
  ...
  $rootScope.player1_name = $rootScope.player1_name;
  $rootScope.player1_wins = $stateParams.player1_wins;
  $rootScope.player2_wins = $stateParams.player2_wins;
...
lab2.controller('player2', function(processMove, $rootScope, $scope, $ionicModal, $ionicLoading, $ionicViewSwitcher, $state, $stateParams, navigation) {
  ...
  $rootScope.player2_name = $rootScope.player2_name;
  $rootScope.player2_wins = $stateParams.player2_wins;
  $rootScope.player1_wins = $stateParams.player1_wins;

  $scope.onNext = function (choice) {

    /* TO DO: Use processMove factory instance to check if player1 has a win */
    ...
    if ($stateParams.player1_wins >= 2) // Check if player1 has best 2/3
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $ionicViewSwitcher.nextDirection('forward');
        /* TO DO: Transition to results state and pass winner since player1 has best 2/3 */
        $state.go('results', {
          'winner': 1
        });
      }
    ...
    else if ($stateParams.player2_wins >= 2) // Check if player2 has best 2/3
    {
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $ionicViewSwitcher.nextDirection('forward');
        /* TO DO: Transition to results state and pass winner since player2 has best 2/3 */
        $state.go('results', {
          'winner': 2
        });
      }
      ...
      if(window.plugins && window.plugins.nativepagetransitions) {
        ...
        $ionicViewSwitcher.nextDirection('forward');
        /* TO DO: Transition to player1 state since neither player has 2/3 */
        $state.go('player1', {
          'player1_choice': 0,
          'player2_choice': 0,
          'player1_wins' : $stateParams.player1_wins,
